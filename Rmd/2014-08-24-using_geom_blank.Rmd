Using geom_blank for better axis ranges
========================================================
...
Using facet_wrap() in **ggplot2** is a great way to create multiple paneled plots.  Though when I am running these, particularly on datasets with different scales, the axis are not as clean as I like.  To provide an illustration of what I am refering to, I will make up some fake data with some different scales in the y values and then plot these in ggplot
...
### coord_cartesian
When using a single panel or data that is all on the same scale, I frequently use ```coord_cartesian```.  Unfortunately when you have panels with different scales this is no longer as useful because it forces everthing on the same scale (which depending on what you are looking at may not be bad).    
```{r}
ggplot() + 
  geom_point(data=foo.dat,aes(x=x,y=y,colour=group),size=4) + 
